...
export default function AdminCreateDocketPage() {
  ...
  const clearExhibitErr = (i, field) => setErrors(p => {
    ...
    const ne = { ...p, exhibits: { ...p.exhibits, [i]: { ...p.exhibits[i], [field]: "" } } };
    return ne;
  });
  ...
  const validate = () => {
    ...
    const hasExhibitErrors = Object.keys(exhibitErrs).length > 0;
    return !hasTopErrors && !hasExhibitErrors;
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await adminAPI.createAdminDocket(submissionData);
      alert("✅ Docket created successfully!");
      router.push("/admin/dockets");
    } catch (error) {
      console.error("Error creating docket:", error);
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea value={docket.response.body}
                  ...
                  rows={10} placeholder="Write the full response here..." />
        ...
        <div className="space-y-6">
          ...
          <div className="border border-[#d4c8b4] bg-white p-4 md:p-6 rounded-lg shadow-sm">
            <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
              <div>
                ...
                <p className="font-mono-dm text-xs text-[#9a8870]">Add key events in chronological order (at least one required)</p>
              </div>
            ...
            {docket.timeline.length === 0 ? (
              <p className="text-center text-[#9a8870] py-8 font-garamond text-sm">No timeline events added. Click "Add Event" to create one.</p>
            ) : (
              docket.timeline.map((entry, i) => (
                <div key={i} className="border-l-2 border-[#b8974a] pl-4 relative mb-6">
          ...
          <div className="border border-[#d4c8b4] bg-white p-4 md:p-6 rounded-lg shadow-sm">
            <div className="flex items-center justify-between mb-1 flex-wrap gap-2">
              <div>
                <h2 className="font-playfair font-bold text-lg text-[#1e2d4a]">
                  Exhibits <span className="text-red-500">*</span>
                </h2>
                <p className="font-mono-dm text-xs text-[#9a8870]">
                  {docket.exhibits.length} / {MAX_FILES} · At least one required
                </p>
              </div>
            ...
            <p className="font-mono-dm text-xs text-[#9a8870] mb-3">Max {MAX_FILES} files · Max {MAX_FILE_SIZE_MB} MB each · PDF, Word, Excel, CSV, JPEG, PNG</p>
                      ...
                      <div className="mt-3">
                        ...
                        {ex.fileUrl ? (
                          <div className="flex items-center justify-between p-2 bg-[#ede8dc] rounded">
                            ...
                            <div className="flex gap-2 flex-shrink-0">
                              <a href={`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}${ex.fileUrl}`}
                                target="_blank" className="text-[#b8974a] text-xs hover:underline cursor-pointer">
                                View
                              </a>
                              <button
                                onClick={() => {
