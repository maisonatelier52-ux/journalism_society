...
//           <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-3 w-full text-[#8a9bb8] hover:bg-[#2a3f6a] hover:text-[#f5f0e8] rounded transition-colors cursor-pointer">
//             <FiLogOut size={18} />
  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      ...
      try {
        ...
        try {
          const data = await adminAPI.getFlagStats();
          setPendingFlags(data.stats?.pending || 0);
        } catch {
