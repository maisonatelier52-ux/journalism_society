...
import Header from "@/components/Header";
import Footer from "@/components/Footer";
import { FiCalendar, FiTag, FiUser, FiArrowLeft, FiArrowRight, FiShare2, FiExternalLink } from "react-icons/fi";
...
export default function PressReleaseDetailPage() {
  ...
  const fetchPressRelease = async () => {
    setLoading(true);
    try {
      const response = await pressReleaseAPI.getPressReleaseById(slug);
      setRelease(response.release);
      
      // Fetch related releases (same category, excluding current)
      if (response.release) {
        const allReleases = await pressReleaseAPI.getAllPressReleases();
        const related = allReleases.releases
          .filter(r => r.category === response.release.category && r._id !== response.release._id)
          .slice(0, 3);
      ...
      <main className="max-w-4xl mx-auto px-6 py-8 pb-20">
        ...
        <div className="mb-8">
          <div className="flex flex-wrap items-center gap-3 mb-4">
            ...
            <div className="flex items-center gap-1.5 text-[#9a8870]">
              <FiUser size={12} />
              <span className="font-garamond text-sm">{release.author || "Journalism Society"}</span>
            </div>
          </div>
        ...
        {release.featuredImage && (
          <div className="mb-10 -mx-6 md:mx-0">
           <img
              src={
                release.featuredImage?.startsWith("/uploads/press-releases/")
                  ? `${process.env.NEXT_PUBLIC_API_URL}${release.featuredImage}`
                  : release.featuredImage
              }
              alt={release.title}
              className="w-full h-auto object-cover rounded-sm"
        ...
        <div className="bg-[#ede8dc] border border-[#d4c8b4] p-5 mb-10">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              <button
                onClick={handleShare}
                className="w-9 h-9 rounded-full border border-[#c4b89a] flex items-center justify-center text-[#9a8870] hover:bg-[#1e2d4a] hover:text-white hover:border-[#1e2d4a] transition-all cursor-pointer"
              >
                <FiShare2 size={14} />
              </button>
            </div>
          ...
          {copied && (
            <p className="text-center text-green-600 text-sm mt-3 font-mono-dm">Link copied to clipboard!</p>
          )}
        </div>
        ...
        {relatedReleases.length > 0 && (
          <div className="mt-10">
            <h3 className="font-playfair font-bold text-2xl text-[#1e2d4a] mb-6">Related Releases</h3>
            <div className="grid md:grid-cols-3 gap-4">
              {relatedReleases.map(related => (
                <Link
                  key={related._id}
                  href={`/press-releases/${related._id}`}
                  className="group block border border-[#d4c8b4] bg-[#faf6ee] p-4 hover:bg-white hover:shadow-md transition-all no-underline"
                >
                  <span className="font-mono-dm text-[0.5rem] tracking-[0.08em] uppercase bg-[#1e2d4a] text-[#f5f0e8] px-2 py-0.5 inline-block mb-2">
                    {related.category}
                  </span>
                  <p className="font-playfair font-bold text-sm text-[#1e2d4a] group-hover:text-[#b8974a] transition-colors line-clamp-2">
                    {related.title}
                  </p>
                  <p className="font-mono-dm text-xs text-[#9a8870] mt-2">{formatDate(related.date)}</p>
                </Link>
              ))}
            </div>
          </div>
        ...
        {/* Back to Top */}
        <div className="mt-12 pt-6 border-t border-[#d4c8b4] flex justify-center">
          <button
            onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
            className="font-mono-dm text-xs uppercase tracking-wider text-[#b8974a] hover:text-[#1e2d4a] transition-colors cursor-pointer"
          >
            ↑ Back to Top
          </button>
        </div>
