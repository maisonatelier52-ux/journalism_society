// // app/admin/press-releases/edit/[id]/page.jsx
// "use client";
...
//     setLoading(true);
//     try {
//       const response = await adminAPI.getPressRelease(params.id);
...
//     setSaving(true);
//     try {
//       await adminAPI.updatePressRelease(params.id, release);
//       alert("✅ Press release updated successfully!");
//       router.push("/admin/press-releases");
//     } catch (error) {
//       console.error("Error updating press release:", error);
//       alert(error.response?.data?.message || "Failed to update press release");
//     } finally {
//       setSaving(false);
//     }
//   };
...
//           </button>
//           <h1 className="font-playfair font-black text-2xl md:text-3xl text-[#1e2d4a]">Edit Press Release</h1>
//           <p className="font-garamond text-[#7a6e5e]">Update the press release content</p>
//         </div>
...
//                 className="px-3 py-2 bg-[#1e2d4a] text-white hover:bg-[#2a3f6a] transition-colors"
//               >
//                 <FiPlus size={14} />
//               </button>
//             </div>
...
//                     key={tag}
//                     onClick={() => addTag(tag)}
//                     className="font-mono-dm text-[0.5rem] tracking-[0.08em] uppercase text-[#7a6e5e] border border-[#c4b89a] px-2 py-0.5 hover:bg-[#ede8dc] transition-colors"
//                   >
//                     {tag}
//                   </button>
//                 ))}
...
import { 
  FiSave, FiArrowLeft, FiPlus, FiTrash2, FiInfo, 
  FiUpload, FiImage, FiX, FiLink, FiAlertCircle
} from "react-icons/fi";
import adminAPI from "@/services/adminApi";
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="border border-[#d4c8b4] bg-white p-4 md:p-6 rounded-lg shadow-sm">
            ...
            <input
              ...
              value={release.title}
              onChange={(e) => setRelease({ ...release, title: e.target.value })}
              className="w-full border border-[#d4c8b4] p-3 font-garamond text-lg focus:outline-none focus:border-[#1e2d4a] text-[#1e2d4a] bg-white rounded"
              placeholder="Enter press release title..."
            />
          </div>
          ...
          <div className="border border-[#d4c8b4] bg-white p-4 md:p-6 rounded-lg shadow-sm">
            ...
            <textarea
              ...
              placeholder="Write the full press release content here..."
            />
            <div className="mt-2 p-3 bg-[#faf6ee] border border-[#d4c8b4] rounded">
              <div className="flex items-center gap-2 mb-2">
                <FiInfo size={14} className="text-[#b8974a]" />
                <p className="font-mono-dm text-xs text-[#9a8870]">Formatting Tips</p>
              </div>
              <p className="font-garamond text-xs text-[#7a6e5e]">
                Use <code className="bg-[#ede8dc] px-1 rounded">&lt;h3&gt;</code> for subheadings,{' '}
                <code className="bg-[#ede8dc] px-1 rounded">&lt;p&gt;</code> for paragraphs,{' '}
                <code className="bg-[#ede8dc] px-1 rounded">&lt;ul&gt;</code> for lists, and{' '}
                <code className="bg-[#ede8dc] px-1 rounded">&lt;blockquote&gt;</code> for quotes.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="border border-[#d4c8b4] bg-white p-4 md:p-6 rounded-lg shadow-sm">
            ...
            <div className="flex gap-2 mb-4 border-b border-[#e4ddd0] pb-3">
              <button
                type="button"
                onClick={() => setImageUploadMethod("url")}
                className={`flex-1 flex items-center justify-center gap-2 py-2 text-sm font-mono-dm uppercase tracking-wider transition-colors cursor-pointer ${
                  imageUploadMethod === "url" 
                    ? "text-[#b8974a] border-b-2 border-[#b8974a]" 
                    : "text-[#9a8870] hover:text-[#1e2d4a]"
                }`}
            ...
            {imageUploadMethod === "upload" && (
              <div>
                <div
                  onClick={() => fileInputRef.current?.click()}
                  className="border-2 border-dashed border-[#c4b89a] bg-[#faf6ee] p-6 text-center cursor-pointer hover:bg-[#ede8dc] transition-colors rounded"
                >
                  <input
                    ref={fileInputRef}
                    type="file"
          ...
          <div className="border border-[#d4c8b4] bg-white p-4 md:p-6 rounded-lg shadow-sm">
            ...
            <div className="flex gap-2 mb-3">
              <input
                ...
                onKeyPress={(e) => e.key === 'Enter' && addTag(tagInput)}
                placeholder="Add a tag..."
                className="flex-1 border border-[#d4c8b4] p-2 font-garamond text-sm focus:outline-none focus:border-[#1e2d4a] text-[#1e2d4a] bg-white rounded"
              />
              <button
                onClick={() => addTag(tagInput)}
                className="px-3 py-2 bg-[#1e2d4a] text-white hover:bg-[#2a3f6a] transition-colors cursor-pointer rounded"
              >
                <FiPlus size={14} />
              </button>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {suggestedTags.filter(t => !release.tags.includes(t)).slice(0, 8).map(tag => (
                  <button
                    key={tag}
                    onClick={() => addTag(tag)}
                    className="font-mono-dm text-[0.5rem] tracking-[0.08em] uppercase text-[#7a6e5e] border border-[#c4b89a] px-2 py-0.5 hover:bg-[#ede8dc] transition-colors cursor-pointer rounded"
                  >
                    {tag}
                  </button>
                ))}
            ...
            {release.tags.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {release.tags.map(tag => (
                    <span key={tag} className="inline-flex items-center gap-1 font-mono-dm text-[0.5rem] tracking-[0.08em] uppercase bg-[#1e2d4a] text-[#f5f0e8] px-2 py-0.5 rounded">
                      {tag}
                      <button onClick={() => removeTag(tag)} className="hover:text-[#b8974a] transition-colors cursor-pointer">
                        <FiTrash2 size={10} />
                      </button>
                    </span>
                  ))}
