...
export default function DocketsPage() {
  ...
  // Get unique types from fetched dockets
      ...
      <main className="max-w-6xl mx-auto px-6 pb-20">
        ...
        <div className="mb-2" style={{ background: "#ede8dc", border: "1px solid #d4c8b4", padding: "14px 20px" }}>
          <div className="flex items-center gap-4 flex-wrap">
            ...
            {/* Status chips */}
            <div className="flex items-center gap-1">
              <span className="font-mono-dm text-xs tracking-widest uppercase mr-1" style={{ color: "#9a8870" }}>Status</span>
              {STATUS_LIST.map(s => (
                <button
                  key={s}
                  onClick={() => setStatus(s)}
            ...
            <div className="flex items-center gap-3 ml-auto">
              ...
              <div className="flex border border-[#c4b89a] overflow-hidden">
                <button
                  onClick={() => setView("list")}
                  className="px-2.5 py-1.5 flex items-center transition-colors cursor-pointer"
                  style={view === "list" ? { background: "#1e2d4a", color: "#f5f0e8" } : { color: "#9a8870" }}
                >
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg>
                </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {filtered.map(d => {
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between">
                    <span className="font-mono-dm text-xs tracking-wider px-2 py-0.5 text-[#f5f0e8]" style={{ background: "#1e2d4a" }}>{d.docketId}</span>
                    <span className={`inline-flex items-center gap-1.5 font-mono-dm text-xs tracking-wider uppercase px-2 py-0.5 border ${st.text} ${st.bg} ${st.border}`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${st.dot}`} />
                      {d.status}
                    </span>
                  ...
                  <div className="border-t pt-3" style={{ borderColor: "#1e2d4a" }}>
                    <div className="font-playfair font-bold text-base leading-snug mb-2" style={{ color: "#1e2d4a" }}>{displayTitle}</div>
                    <p className="font-garamond text-sm leading-relaxed line-clamp-3" style={{ color: "#7a6e5e" }}>{summaryText}</p>
                  </div>
                  <div className="mt-auto pt-3 flex items-end justify-between border-t" style={{ borderColor: "#e4ddd0" }}>
                    <div>
                      <div className="font-garamond italic text-xs" style={{ color: "#9a8870" }}>{respondentName}</div>
                      <div className="font-mono-dm text-xs mt-0.5" style={{ color: "#b8b0a0" }}>{fmtDate(filedDate)} · {exhibitCount} exhibits</div>
                    </div>
