  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f0e8" }}>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 pb-20">
        {/* Breadcrumb */}
        <p className="font-mono-dm text-xs tracking-widest uppercase mb-4" style={{ color: "#9a8870" }}>
          Public Record / <Link href="/document-room" className="hover:text-[#b8974a]">Document Room</Link> / {doc.documentId || doc.id}
        </p>
        ...
        <div className="flex flex-wrap items-center gap-5 mb-8 pb-4 border-b border-[#d4c8b4]">
          <div>
            <p className="font-mono-dm text-xs tracking-widest uppercase mb-1" style={{ color: "#9a8870" }}>Type</p>
            <span className="font-mono-dm text-sm px-2 py-0.5 border" style={{ color: "#7a6e5e", borderColor: "#c4b89a" }}>{doc.type}</span>
          </div>
          <div>
            <p className="font-mono-dm text-xs tracking-widest uppercase mb-1" style={{ color: "#9a8870" }}>Date</p>
            <p className="font-garamond text-sm" style={{ color: "#1e2d4a" }}>{fmtDate(doc.createdAt || doc.publishedDate)}</p>
          </div>
          {doc.sourceDocketNumber && (
            <div>
              <p className="font-mono-dm text-xs tracking-widest uppercase mb-1" style={{ color: "#9a8870" }}>Source Docket</p>
              <Link href={`/dockets/${doc.sourceDocketId}`} className="font-mono-dm text-sm text-[#b8974a] hover:underline">
                {doc.sourceDocketNumber}
              </Link>
            </div>
          )}
          <div className="ml-auto">
            <button
              ...
              className="inline-flex items-center gap-2 font-mono-dm text-xs uppercase tracking-wider px-4 py-2 bg-[#1e2d4a] text-[#f5f0e8] hover:opacity-90 transition disabled:opacity-50 cursor-pointer"
            >
              {downloading ? (
                <>
        ...
        {downloadSuccess && (
          <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded flex items-center gap-2">
            ...
            <p className="font-garamond text-sm text-green-700">Document downloaded successfully!</p>
          </div>
        )}
        ...
        {downloadError && (
          <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded flex items-center gap-2">
            ...
            <p className="font-garamond text-sm text-red-700">Download failed: {downloadError}</p>
          </div>
        )}
        ...
        <div className="preview-container rounded-sm overflow-hidden">
          <div className="preview-header px-4 py-2 flex items-center justify-between">
            <span className="font-mono-dm text-xs uppercase tracking-wider" style={{ color: "#9a8870" }}>Document Preview</span>
            <span className="font-mono-dm text-xs" style={{ color: "#9a8870" }}>{doc.fileType?.split('/').pop() || 'PDF'}</span>
          </div>
          <div className="p-4 flex-1 bg-[#fefcf8]">
            <div className="border border-[#e4ddd0] rounded-sm bg-white p-6 shadow-sm">
              <div className="flex items-start gap-4">
                <div className="flex-shrink-0 w-12 h-12 bg-[#ede8dc] rounded flex items-center justify-center">
                  <FiFileText size={28} className="text-[#b8974a]" />
                </div>
                <div className="flex-1">
                  <p className="font-playfair font-bold text-md md:text-lg mb-1" style={{ color: "#1e2d4a" }}>{doc.title}</p>
                  <p className="font-garamond text-xs md:text-sm mb-3" style={{ color: "#7a6e5e" }}>{doc.description || doc.preview || "No description available."}</p>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-[#ede8dc] p-4 border border-[#d4c8b4]">
            <p className="font-mono-dm text-xs uppercase tracking-wider mb-2" style={{ color: "#9a8870" }}>Source</p>
            <p className="font-garamond text-sm" style={{ color: "#1e2d4a" }}>Journalism Society Public Record</p>
            <p className="font-mono-dm text-xs mt-1" style={{ color: "#9a8870" }}>Added: {fmtDate(doc.createdAt)}</p>
            {doc.fileSize && (
              <p className="font-mono-dm text-xs mt-1" style={{ color: "#9a8870" }}>Size: {(doc.fileSize / 1024).toFixed(1)} KB</p>
            )}
          </div>
        </div>
