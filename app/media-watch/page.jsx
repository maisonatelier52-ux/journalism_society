  ...
  return (
    <div className="flex border border-[#d4c8b4] bg-[#faf6ee] overflow-hidden">
      ...
      <div className="flex-1 p-4 flex flex-col gap-3">
        ...
        <div className="border-t border-[#e4ddd0] pt-3 flex flex-col gap-2">
...
/* ── SUBMIT CITATION MODAL (User facing - with searchable dropdown) ── */
function SubmitCitationModal({ onClose, onSuccess, dockets }) {
  ...
  // Filter dockets based on search input (matches both ID and title)
      ...
      <div className="w-full max-w-[560px] bg-[#f5f0e8] border-t-4 border-[#b8974a] max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        {submitted ? (
          <div className="success-pop p-10 text-center">
            ...
            <button onClick={onClose} className="font-mono-dm bg-[#1e2d4a] text-[#f5f0e8] px-6 py-2.5 text-[0.6rem] tracking-[0.12em] uppercase cursor-pointer">
              Close
            </button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="p-6">
            <p className="font-garamond text-[0.92rem] italic text-[#7a6e5e] leading-relaxed mb-5 border-l-2 border-[#b8974a] pl-3">
              Know of coverage that isn't listed? Submit it for editorial review.
            </p>

            {/* Docket Selection with Searchable Dropdown */}
              ...
              <div className="relative">
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full bg-[#faf6ee] border pl-10 pr-8 p-2.5 font-garamond text-[0.97rem] text-[#1e2d4a] focus:outline-none focus:border-[#1e2d4a] ${errors.docketId ? "border-[#b8190c]" : "border-[#d4c8b4]"}`}
                  />
                  {docketSearch && (
                    <button
                      type="button"
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1fr_130px] gap-2.5 mb-4">
              ...
              <div>
                ...
                <select
                  className="w-full bg-[#faf6ee] border border-[#d4c8b4] p-2.5 font-garamond text-[0.97rem] text-[#1e2d4a] focus:outline-none focus:border-[#1e2d4a] cursor-pointer"
                  value={form.type}
                  onChange={(e) => setForm({ ...form, type: e.target.value })}
                >
                  {["Original Report", "Follow-up", "Opinion", "Fact-Check", "News", "Regional", "Other"].map((t) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1fr_130px] gap-2.5 mb-4">
              ...
              <div>
                ...
                <input
                  type="date"
                  className={`w-full bg-[#faf6ee] border p-2.5 font-garamond text-[0.97rem] text-[#1e2d4a] focus:outline-none focus:border-[#1e2d4a] cursor-pointer ${errors.date ? "border-[#b8190c]" : "border-[#d4c8b4]"}`}
                  value={form.date}
                  onChange={(e) => setForm({ ...form, date: e.target.value })}
                />
                {errors.date && <p className="font-mono-dm text-[0.54rem] text-[#b8190c] mt-1">{errors.date}</p>}
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#0a0f1e]/90 flex items-center justify-center p-5" onClick={onClose}>
      <div className="w-full max-w-[480px] bg-[#f5f0e8]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-5">
          <p className="font-mono-dm text-[0.54rem] text-[#9a8870] tracking-[0.1em] uppercase mb-2.5">{fmtDate(item.date)}</p>
          <h3 className="font-playfair font-bold text-[1.15rem] leading-tight text-[#1e2d4a] mb-3.5">{item.headline}</h3>
          <div className="bg-[#ede8dc] border border-[#d4c8b4] p-3.5 mb-4">
            <p className="font-mono-dm text-[0.52rem] tracking-[0.12em] uppercase text-[#9a8870] mb-1.5">Summary</p>
            <p className="font-garamond text-[0.97rem] leading-relaxed text-[#4a4035]">{item.summary || "No summary available."}</p>
          </div>
          ...
          <p className="font-mono-dm text-[0.5rem] text-[#b8b0a0] tracking-[0.08em] mt-2 text-center">Opens in new tab · External content</p>
        </div>
      </div>
    </div>
  );
...
function Toast({ message, type, onClose }) {
  return (
    <div className="fixed bottom-5 right-5 z-50 flex items-center gap-2.5 px-5 py-3 shadow-lg toast-animation"
...
function ErrorState({ onRetry }) {
  return (
    <div className="text-center py-20 border border-[#d4c8b4] bg-[#faf6ee]">
      <div className="w-12 h-12 rounded-full bg-[#fee2e2] flex items-center justify-center mx-auto mb-4">
        <FiAlertCircle size={22} className="text-[#b8190c]" />
      </div>
      <p className="font-playfair font-bold text-[1.2rem] text-[#1e2d4a] mb-2">Failed to load media</p>
      <p className="font-garamond text-[0.97rem] italic text-[#9a8870] mb-6">Unable to reach the server. Please check your connection.</p>
      <button onClick={onRetry}
        className="font-mono-dm text-[0.6rem] tracking-[0.12em] uppercase bg-[#1e2d4a] text-[#f5f0e8] px-6 py-2.5 flex items-center gap-2 mx-auto hover:bg-[#2a3f6a] transition-colors cursor-pointer">
        <FiRefreshCw size={12} />
        Try Again
      </button>
    </div>
...
export default function MediaWatchPage() {
  ...
  /* ── Fetch dockets for dropdown from docketsAPI ── */
  ...
  // Format dockets for dropdown - using docketsList from API
  ...
  return (
    <div className="min-h-screen bg-[#f5f0e8]">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-px bg-white/10">
            {[
              ...
              <div key={s.label} className="bg-[#1e2d4a]/60 p-4">
                <div className="h-0.5 mb-3" style={{ background: s.accent }} />
                {loading ? (
                  <div className="skeleton h-8 w-10 mb-1.5 rounded" />
                ) : (
                  <div className="font-playfair font-black text-3xl leading-none" style={{ color: s.accent }}>{s.val}</div>
                )}
                <div className="font-mono-dm text-[0.52rem] tracking-[0.12em] uppercase text-[#3a4e6a] mt-1.5">{s.label}</div>
              ...
              <div className="ticker-track">
                {[...tickerItems, ...tickerItems].map((item, i) => (
                  <button key={i} onClick={() => setPreviewItem(item)}
                    className="bg-transparent border-none cursor-pointer flex items-center gap-0 p-0 flex-shrink-0">
                    <span className="px-5 pl-6 font-mono-dm text-[0.58rem] text-[#8a9bb8] tracking-[0.05em] whitespace-nowrap">{item.outlet}</span>
                    <span className="font-mono-dm text-[0.6rem] text-[#3a4e6a]">·</span>
                    <span className="px-6 pl-5 font-garamond text-[0.82rem] text-[#c8bfa8] whitespace-nowrap">
                      {(item.headline || "").length > 72 ? item.headline.slice(0, 72) + "…" : item.headline}
                    </span>
      ...
      <main className="max-w-6xl mx-auto px-6 py-9 pb-20">
        ...
        <div className="bg-[#ede8dc] border border-[#d4c8b4] p-3.5 mb-1">
          <div className="flex flex-col md:flex-row items-center gap-4 flex-wrap">
            ...
            <div className="flex gap-1.5 items-center flex-wrap">
              ...
              {uniqueStances.map((s) => (
                <button key={s} onClick={() => setStanceFilter(s)}
                  className={`font-mono-dm text-[0.58rem] tracking-[0.12em] uppercase px-3 py-1 border transition-all whitespace-nowrap cursor-pointer ${
                    stanceFilter === s
                      ? "bg-[#1e2d4a] text-[#f5f0e8] border-[#1e2d4a]"
                      : "bg-transparent text-[#7a6e5e] border-[#c4b89a] hover:border-[#9a8870] hover:text-[#3a3028]"
                  }`}>
            ...
            <div className="flex items-center gap-3 ml-auto">
                ...
                {[["newest", "Latest"], ["oldest", "Oldest"]].map(([val, label]) => (
                  <React.Fragment key={val}>
                    <button onClick={() => setSort(val)}
                      className={`font-mono-dm text-[0.56rem] tracking-[0.1em] uppercase cursor-pointer ${sortBy === val ? "text-[#1e2d4a] font-medium" : "text-[#9a8870]"} hover:text-[#1e2d4a] transition-colors`}>
                      {label}
                    </button>
                  </React.Fragment>
                ))}
              ...
              <div className="flex border border-[#c4b89a] overflow-hidden">
                {[
                  { k: "cards", icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7" /><rect x="14" y="3" width="7" height="7" /><rect x="3" y="14" width="7" height="7" /><rect x="14" y="14" width="7" height="7" /></svg> },
                  { k: "list",  icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="3" y1="6" x2="21" y2="6" /><line x1="3" y1="12" x2="21" y2="12" /><line x1="3" y1="18" x2="21" y2="18" /></svg> },
                ].map(({ k, icon }) => (
                  <button key={k} onClick={() => setView(k)}
                    className={`p-1.5 flex items-center transition-colors cursor-pointer ${view === k ? "bg-[#1e2d4a] text-[#f5f0e8]" : "bg-transparent text-[#9a8870]"}`}>
                    {icon}
                  </button>
                ))}
              </div>
        ...
        {/* ── Secondary filter row (Docket / Type / Outlet) ── */}
        <div className="py-2.5 pb-4 flex gap-1.5 flex-wrap items-center">
          <span className="font-mono-dm text-[0.52rem] tracking-[0.12em] uppercase text-[#9a8870] mr-0.5">Docket</span>
          {uniqueDocketIds.slice(0, 5).map((d) => (
            <button key={d} onClick={() => setDocketFilter(d)}
              className={`font-mono-dm text-[0.58rem] tracking-[0.12em] uppercase px-3 py-1 border transition-all cursor-pointer ${
                docketFilter === d
                  ? "bg-[#1e2d4a] text-[#f5f0e8] border-[#1e2d4a]"
                  : "bg-transparent text-[#7a6e5e] border-[#c4b89a] hover:border-[#9a8870] hover:text-[#3a3028]"
              }`}>
          ...
          {uniqueDocketIds.length > 5 && (
            <select onChange={(e) => setDocketFilter(e.target.value)} value={docketFilter}
              className="font-mono-dm text-[0.58rem] tracking-[0.08em] uppercase border border-[#c4b89a] px-2 py-1 pl-2 pr-7 cursor-pointer outline-none bg-[#faf6ee] text-[#7a6e5e] appearance-none"
              style={{ backgroundImage: "url(\"data:image/svg+xml,%3Csvg width='10' height='6' viewBox='0 0 10 6' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1L5 5L9 1' stroke='%23c4b89a' stroke-width='1.5' fill='none'/%3E%3C/svg%3E\")", backgroundRepeat: "no-repeat", backgroundPosition: "right 8px center" }}>
          ...
          {hasFilters && (
            <button onClick={clearAll}
              className="font-mono-dm text-[0.56rem] tracking-[0.1em] uppercase border border-dashed border-[#b8974a] text-[#b8974a] px-3 py-1 ml-1 hover:bg-[#ede8dc] transition-colors cursor-pointer">
              Reset all
            </button>
          )}

          <span className="font-mono-dm text-[0.56rem] tracking-[0.1em] uppercase text-[#9a8870] ml-auto">
            {loading ? "Loading…" : `${filtered.length} of ${coverage.length} articles`}
          </span>
        </div>
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="text-center py-20 border border-[#d4c8b4]">
            <p className="font-playfair italic text-[1.8rem] text-[#c4b89a] mb-2.5">
              {coverage.length === 0 ? "No media coverage yet" : "No coverage found"}
            </p>
            <p className="font-garamond text-base text-[#9a8870]">
              {coverage.length === 0
            ...
            {coverage.length === 0 && (
              <button onClick={() => setShowSubmit(true)}
                className="mt-6 font-mono-dm text-[0.6rem] tracking-[0.12em] uppercase bg-[#1e2d4a] text-[#f5f0e8] px-6 py-2.5 inline-flex items-center gap-2 hover:bg-[#2a3f6a] transition-colors cursor-pointer">
                <FiPlus size={11} />
                Submit a Media Citation
              </button>
            )}
                    ...
                    <div className="mt-auto pt-2.5 border-t border-[#e4ddd0] flex justify-between items-center">
                      {item.docketId ? (
                        <Link href={`/dockets/${item.docketId}`} onClick={(e) => e.stopPropagation()}
                          className="font-mono-dm text-[0.52rem] tracking-[0.1em] uppercase text-[#b8974a] no-underline">
                          {item.docketId} →
                        </Link>
                      ) : (
                        <span className="font-mono-dm text-[0.52rem] text-[#c4b89a]">No docket linked</span>
                      )}
                      <FiArrowRight size={13} className="text-[#1e2d4a] opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all" />
                    </div>
                  </div>
              ...
              return (
                <div key={item._id || item.id || idx}
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="inline-flex items-center gap-1">
                      <span className="w-1.5 h-1.5 rounded-full" style={{ background: sc.color }} />
                      <span className="font-mono-dm text-[0.5rem] uppercase" style={{ color: sc.color, letterSpacing: "0.08em" }}>{sc.label}</span>
                    </span>
                  </div>

                  <div>
                    <p className="font-playfair font-bold text-[0.97rem] leading-tight text-[#1e2d4a] mb-1">{item.headline}</p>
                    <p className="font-garamond text-[0.84rem] text-[#9a8870] truncate">{(item.summary || "").slice(0, 90)}{item.summary?.length > 90 ? "…" : ""}</p>
                  </div>
                  ...
                  {item.docketId ? (
                    <Link href={`/dockets/${item.docketId}`} onClick={(e) => e.stopPropagation()}
                      className="font-mono-dm text-[0.58rem] text-[#b8974a] no-underline tracking-[0.06em] hidden md:block">
                      {item.docketId}
                    </Link>
                  ) : (
                    <span className="font-mono-dm text-[0.58rem] text-[#c4b89a] hidden md:block">—</span>
                  )}
        ...
        {!loading && !error && filtered.length > 0 && (
          <div className="mt-14 pt-7 border-t-2 border-[#1e2d4a] flex items-center justify-between flex-wrap gap-4">
            ...
            <button onClick={() => setShowSubmit(true)}
              className="font-mono-dm text-[0.6rem] tracking-[0.12em] uppercase bg-[#1e2d4a] text-[#f5f0e8] px-6 py-3.5 flex items-center gap-2.5 hover:bg-[#2a3f6a] transition-colors cursor-pointer">
              <FiPlus size={11} />
              Submit a Media Citation
            </button>
          </div>
