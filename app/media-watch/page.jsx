...
// import {
//   FiSearch, FiArrowRight, FiCheck, FiAlertCircle, FiX,
//   FiExternalLink, FiPlus, FiRefreshCw, FiChevronLeft, FiChevronRight,
// } from "react-icons/fi";
...
//     @keyframes successPop { 0% { opacity: 0; transform: scale(0.85); } 60% { transform: scale(1.04); } 100% { opacity: 1; transform: scale(1); } }
//     .success-pop { animation: successPop 0.35s ease forwards; }
//     @keyframes shimmer { 0% { background-position: -600px 0; } 100% { background-position: 600px 0; } }
//     .skeleton { background: linear-gradient(90deg, #e4ddd0 25%, #ede8dc 50%, #e4ddd0 75%); background-size: 600px 100%; animation: shimmer 1.4s infinite; border-radius: 2px; }
//     .filter-scroll::-webkit-scrollbar { height: 3px; }
//     .filter-scroll::-webkit-scrollbar-track { background: #e4ddd0; }
//     .filter-scroll::-webkit-scrollbar-thumb { background: #b8974a; border-radius: 2px; }
//     .docket-dropdown::-webkit-scrollbar { width: 6px; }
//     .docket-dropdown::-webkit-scrollbar-thumb { background: #c4b89a; border-radius: 3px; }
//     .page-btn { transition: all 0.15s; }
//     .page-btn:hover:not(:disabled) { background: #1e2d4a !important; color: #f5f0e8 !important; border-color: #1e2d4a !important; }
//   `}</style>
// );
...
//   return (
//     <div className="mt-8 pt-5 border-t border-[#d4c8b4] flex flex-col sm:flex-row items-center justify-between gap-4">
...
//         >
//           <FiChevronLeft size={12} /><span className="hidden sm:inline">Prev</span>
//         </button>
//         <div className="flex items-center gap-0.5 mx-1">
//           {getPages().map((page, i) =>
//             page === "..." ? (
//               <span key={`e-${i}`} className="font-mono-dm text-xs text-[#c4b89a] px-2 py-1.5">…</span>
//             ) : (
//               <button key={page} onClick={() => onPageChange(page)}
//                 className={`page-btn w-8 h-8 font-mono-dm text-xs border cursor-pointer ${currentPage === page ? "bg-[#1e2d4a] text-[#f5f0e8] border-[#1e2d4a]" : "border-[#d4c8b4] text-[#7a6e5e]"}`}
//               >{page}</button>
//             )
//           )}
//         </div>
...
//           className="page-btn flex items-center gap-1 px-3 py-1.5 border border-[#d4c8b4] font-mono-dm text-xs text-[#7a6e5e] disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer"
//         >
//           <span className="hidden sm:inline">Next</span><FiChevronRight size={12} />
//         </button>
//       </div>
//     </div>
//   );
...
//         <div className="flex justify-between"><div className="skeleton h-3 w-16 sm:h-4 sm:w-20 rounded" /><div className="skeleton h-3 w-12 sm:h-4 sm:w-16 rounded" /></div>
...
//           <div className="skeleton h-2 w-full rounded" /><div className="skeleton h-2 w-3/4 rounded" />
//         </div>
//         <div className="mt-auto pt-2 border-t border-[#e4ddd0]"><div className="skeleton h-2 w-20 sm:h-3 sm:w-24 rounded" /></div>
//       </div>
//     </div>
//   );
// }
...
//   const [form, setForm] = useState({ publication: "", type: "Follow-up", url: "", date: "", headline: "", docketId: "", note: "" });
...
//     if (errors.docketId) setErrors({ ...errors, docketId: "" });
//   };
...
//     try {
//       const result = await mediaAPI.submitCitation({ outlet: form.publication, headline: form.headline, url: form.url, date: form.date, type: form.type, note: form.note, docketId: form.docketId });
...
//   useEffect(() => {
//     const handleClickOutside = (e) => { if (showDropdown && !e.target.closest(".docket-search-container")) setShowDropdown(false); };
...
//               <FiCheck size={20} className="text-[#f5f0e8]" />
//             </div>
//             <p className="font-playfair font-bold text-base sm:text-[1.1rem] text-[#1e2d4a] mb-2">Citation Submitted</p>
//             <p className="font-garamond text-sm sm:text-[0.95rem] leading-relaxed text-[#6a5e4e] mb-5"><strong>{form.publication}</strong>'s coverage has been flagged for editorial review.</p>
//             <button onClick={onClose} className="font-mono-dm bg-[#1e2d4a] text-[#f5f0e8] px-5 sm:px-6 py-2 sm:py-2.5 text-[0.55rem] sm:text-[0.6rem] tracking-[0.12em] uppercase cursor-pointer">Close</button>
//           </div>
//         ) : (
...
//                   <input type="text" value={docketSearch} onChange={(e) => { setDocketSearch(e.target.value); setShowDropdown(true); if (!e.target.value) { setSelectedDocket(null); setForm({ ...form, docketId: "" }); } }}
//                     onFocus={() => setShowDropdown(true)} placeholder="Type docket ID or title to search..."
...
//                   />
//                   {docketSearch && <button type="button" onClick={() => { setDocketSearch(""); setSelectedDocket(null); setForm({ ...form, docketId: "" }); setShowDropdown(false); }} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#9a8870] hover:text-[#1e2d4a] cursor-pointer"><FiX size={14} /></button>}
//                 </div>
...
//                     {filteredDockets.length === 0 ? <div className="p-3 text-center font-garamond text-[#9a8870]">No dockets found</div> : filteredDockets.map((docket) => (
//                       <div key={docket.id} onClick={() => selectDocket(docket)} className="p-3 hover:bg-[#ede8dc] cursor-pointer border-b border-[#e4ddd0] last:border-0 transition-colors">
...
//               {selectedDocket && <p className="font-mono-dm text-xs text-[#2d6a4f] mt-1 flex items-center gap-1"><FiCheck size={12} /> Selected: {selectedDocket.id}</p>}
//             </div>
...
//             <span className="px-1.5 py-0.5 border text-[0.52rem] tracking-[0.08em] uppercase" style={{ borderColor: tc.border, background: tc.bg, color: tc.text }}>{item.type}</span>
...
//             </span>
//           </div>
//           <button onClick={onClose} className="text-[#b8b0a0] hover:text-[#1e2d4a] transition-colors cursor-pointer"><FiX size={15} /></button>
//         </div>
//         <div className="p-4 sm:p-5">
//           <p className="font-mono-dm text-[0.54rem] text-[#9a8870] tracking-[0.1em] uppercase mb-2">{fmtDate(item.date)}</p>
...
//               </div>
//               <Link href={`/dockets/${item.docketId}`} onClick={onClose} className="font-mono-dm text-[0.56rem] tracking-[0.1em] uppercase text-[#b8974a] no-underline">View Docket →</Link>
//             </div>
//           )}
...
//             <span className="flex items-center gap-1.5"><span className="font-mono-dm text-[0.54rem] text-[#6a7a94]">{item.outlet}</span><FiExternalLink size={10} className="text-[#6a7a94]" /></span>
//           </a>
//         </div>
//       </div>
//     </div>
...
//   return (
//     <div className="fixed bottom-5 right-5 z-50 flex items-center gap-2.5 px-4 sm:px-5 py-2.5 sm:py-3 shadow-lg toast-animation" style={{ background: type === "success" ? "#2d6a4f" : "#b8190c", color: "#fff" }}>
//       {type === "success" ? <FiCheck size={14} /> : <FiAlertCircle size={14} />}
//       <span className="font-mono-dm text-sm tracking-[0.05em]">{message}</span>
...
//       <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-[#fee2e2] flex items-center justify-center mx-auto mb-4"><FiAlertCircle size={18} className="text-[#b8190c]" /></div>
...
//       <button onClick={onRetry} className="font-mono-dm text-[0.6rem] tracking-[0.12em] uppercase bg-[#1e2d4a] text-[#f5f0e8] px-5 sm:px-6 py-2 sm:py-2.5 flex items-center gap-2 mx-auto hover:bg-[#2a3f6a] transition-colors cursor-pointer">
...
//       setCoverage(Array.isArray(data) ? data : (data.media || data.data || []));
//     } catch { setError(true); }
...
//       setDocketsList(Array.isArray(response) ? response : (response.dockets || []));
//     } catch { setDocketsList([]); }
//     finally { setLoadingDockets(false); }
//   };

//   useEffect(() => { fetchMedia(); fetchDockets(); }, []);

//   // Reset page on filter change
//   useEffect(() => { setCurrentPage(1); }, [search, docketFilter, outletFilter, typeFilter, stanceFilter, sortBy, view]);

//   const uniqueOutlets  = useMemo(() => ["All", ...Array.from(new Set(coverage.map((c) => c.outlet))).sort()], [coverage]);
//   const uniqueDocketIds = useMemo(() => { const ids = new Set(coverage.map((c) => c.docketId).filter(Boolean)); return ["All", ...Array.from(ids)]; }, [coverage]);
//   const uniqueTypes    = useMemo(() => ["All", ...Array.from(new Set(coverage.map((c) => c.type).filter(Boolean)))], [coverage]);
...
//     let l = [...coverage];
//     if (search.trim()) { const q = search.toLowerCase(); l = l.filter((c) => (c.headline || "").toLowerCase().includes(q) || (c.outlet || "").toLowerCase().includes(q) || (c.docketId || "").toLowerCase().includes(q) || (c.summary || "").toLowerCase().includes(q)); }
...
//   const itemsPerPage = view === "cards" ? CARDS_PER_PAGE : LIST_PER_PAGE;
...
//   const handleCitationSuccess = () => { showToast("Citation submitted! It will appear after editorial review.", "success"); fetchMedia(); };
//   const tickerItems = coverage.slice(0, 6);
//   const formattedDockets = useMemo(() => docketsList.map(d => ({ id: d._id || d.id, title: d.response?.title || d.title || d.docketId || "Untitled" })), [docketsList]);

//   return (
//     <div className="min-h-screen bg-[#f5f0e8]">
//       <FontStyle />
//       {previewItem && <ArticlePreviewModal item={previewItem} onClose={() => setPreviewItem(null)} />}
//       {showSubmit && <SubmitCitationModal onClose={() => setShowSubmit(false)} onSuccess={handleCitationSuccess} dockets={formattedDockets} />}
...
//                   <button key={i} onClick={() => setPreviewItem(item)} className="bg-transparent border-none cursor-pointer flex items-center gap-0 p-0 flex-shrink-0">
//                     <span className="px-3 sm:px-5 pl-4 sm:pl-6 font-mono-dm text-[0.58rem] text-[#8a9bb8] tracking-[0.05em] whitespace-nowrap">{item.outlet}</span>
//                     <span className="font-mono-dm text-[0.6rem] text-[#3a4e6a]">·</span>
//                     <span className="px-4 sm:px-6 pl-3 sm:pl-5 font-garamond text-[0.82rem] text-[#c8bfa8] whitespace-nowrap">{(item.headline || "").length > 60 ? item.headline.slice(0, 60) + "…" : item.headline}</span>
...
//               <input type="text" className="w-full bg-[#faf6ee] border-b-2 border-[#1e2d4a] pl-8 pr-8 py-2 font-garamond text-sm sm:text-base text-[#1e2d4a] focus:outline-none" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by headline, outlet, docket ID…" />
//               {search && <button onClick={() => setSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#9a8870] hover:text-[#1e2d4a] cursor-pointer"><FiX size={14} /></button>}
//             </div>
...
//                     <button onClick={() => setSort(val)} className={`font-mono-dm text-[0.56rem] tracking-[0.1em] uppercase cursor-pointer ${sortBy === val ? "text-[#1e2d4a] font-medium" : "text-[#9a8870]"} hover:text-[#1e2d4a] transition-colors`}>{label}</button>
//                   </React.Fragment>
//                 ))}
//               </div>
//               <div className="flex border border-[#c4b89a] overflow-hidden">
//                 {[
//                   { k: "cards", icon: <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7" /><rect x="14" y="3" width="7" height="7" /><rect x="3" y="14" width="7" height="7" /><rect x="14" y="14" width="7" height="7" /></svg> },
//                   { k: "list",  icon: <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="3" y1="6" x2="21" y2="6" /><line x1="3" y1="12" x2="21" y2="12" /><line x1="3" y1="18" x2="21" y2="18" /></svg> },
//                 ].map(({ k, icon }) => (
//                   <button key={k} onClick={() => setView(k)} className="p-1.5 flex items-center transition-colors cursor-pointer" style={view === k ? { background: "#1e2d4a", color: "#f5f0e8" } : { background: "transparent", color: "#9a8870" }}>{icon}</button>
//                 ))}
//               </div>
//             </div>
//           </div>
...
//           <div className="w-px h-3 bg-[#d4c8b4] mx-1" />
//           <span className="font-mono-dm text-[0.52rem] tracking-[0.12em] uppercase text-[#9a8870] mr-0.5 flex-shrink-0">Outlet</span>
//           <select onChange={(e) => setOutletFilter(e.target.value)} value={outletFilter} className="font-mono-dm text-[0.58rem] uppercase border border-[#c4b89a] px-2 py-1 pl-2 pr-6 cursor-pointer outline-none bg-[#faf6ee] text-[#7a6e5e] appearance-none flex-shrink-0" style={{ backgroundImage: "url(\"data:image/svg+xml,%3Csvg width='10' height='6' viewBox='0 0 10 6' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 1L5 5L9 1' stroke='%23c4b89a' stroke-width='1.5' fill='none'/%3E%3C/svg%3E\")", backgroundRepeat: "no-repeat", backgroundPosition: "right 6px center" }}>{uniqueOutlets.map((o) => <option key={o}>{o}</option>)}</select>
//           {hasFilters && <button onClick={clearAll} className="font-mono-dm text-[0.56rem] tracking-[0.1em] uppercase border border-dashed border-[#b8974a] text-[#b8974a] px-2 sm:px-3 py-1 ml-1 hover:bg-[#ede8dc] transition-colors cursor-pointer whitespace-nowrap flex-shrink-0">Reset</button>}
//           <span className="hidden sm:block font-mono-dm text-[0.56rem] tracking-[0.1em] uppercase text-[#9a8870] ml-auto flex-shrink-0">{loading ? "Loading…" : `${filtered.length} of ${coverage.length}`}</span>
//         </div>

//         {/* Loading skeleton */}
//         {loading && <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">{Array.from({ length: 6 }).map((_, i) => <SkeletonCard key={i} />)}</div>}
//         {!loading && error && <ErrorState onRetry={fetchMedia} />}
//         {!loading && !error && filtered.length === 0 && (
//           <div className="text-center py-16 sm:py-20 border border-[#d4c8b4]">
//             <p className="font-playfair italic text-xl sm:text-[1.8rem] text-[#c4b89a] mb-2">{coverage.length === 0 ? "No media coverage yet" : "No coverage found"}</p>
//             <p className="font-garamond text-sm sm:text-base text-[#9a8870]">{coverage.length === 0 ? "Be the first to submit a media citation." : "Try adjusting your filters or search terms."}</p>
//             {coverage.length === 0 && <button onClick={() => setShowSubmit(true)} className="mt-5 sm:mt-6 font-mono-dm text-[0.6rem] tracking-[0.12em] uppercase bg-[#1e2d4a] text-[#f5f0e8] px-5 sm:px-6 py-2 sm:py-2.5 inline-flex items-center gap-2 hover:bg-[#2a3f6a] transition-colors cursor-pointer"><FiPlus size={11} />Submit a Media Citation</button>}
//           </div>
//         )}
...
//                 const tc = TYPE_COLORS[item.type] || TYPE_COLORS["News"];
//                 return (
//                   <div key={item._id || item.id || idx} onClick={() => setPreviewItem(item)} className="flex border border-[#d4c8b4] bg-[#faf6ee] cursor-pointer hover:bg-white hover:shadow-lg hover:-translate-y-0.5 transition-all overflow-hidden group">
...
//                         <FiArrowRight size={11} className="text-[#1e2d4a] opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all" />
//                       </div>
//                     </div>
//                   </div>
//                 );
//               })}
//             </div>
//             <div className="mt-4 text-center sm:hidden"><span className="font-mono-dm text-xs text-[#9a8870]">{filtered.length} of {coverage.length} articles</span></div>
//             {/* ── PAGINATION ── */}
//             <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={handlePageChange} totalItems={filtered.length} itemsPerPage={itemsPerPage} />
//           </>
//         )}
...
//                   const tc = TYPE_COLORS[item.type] || TYPE_COLORS["News"];
//                   return (
//                     <div key={item._id || item.id || idx} onClick={() => setPreviewItem(item)} className="grid grid-cols-[80px_1fr_140px_90px_70px_24px] gap-3 items-center py-3 border-b border-[#e4ddd0] cursor-pointer hover:bg-[#ede8dc] transition-colors group">
//                       <div className="flex flex-col gap-1">
//                         <span className="px-1.5 py-0.5 border inline-block text-[0.5rem] tracking-[0.08em] uppercase w-fit" style={{ borderColor: tc.border, background: tc.bg, color: tc.text }}>{item.type}</span>
//                         <span className="inline-flex items-center gap-1"><span className="w-1 h-1 rounded-full" style={{ background: sc.color }} /><span className="font-mono-dm text-[0.45rem] uppercase" style={{ color: sc.color, letterSpacing: "0.08em" }}>{sc.label}</span></span>
//                       </div>
...
//                       {item.docketId ? <Link href={`/dockets/${item.docketId}`} onClick={(e) => e.stopPropagation()} className="font-mono-dm text-[0.55rem] text-[#b8974a] no-underline tracking-[0.06em] truncate">{item.docketId}</Link> : <span className="font-mono-dm text-[0.55rem] text-[#c4b89a]">—</span>}
...
//             </div>
//             <button onClick={() => setShowSubmit(true)} className="font-mono-dm text-[0.6rem] tracking-[0.12em] uppercase bg-[#1e2d4a] text-[#f5f0e8] px-5 sm:px-6 py-2.5 sm:py-3.5 flex items-center gap-2 hover:bg-[#2a3f6a] transition-colors cursor-pointer"><FiPlus size={11} />Submit a Media Citation</button>
//           </div>
//         )}
//       </main>
//       <Footer />
...
// app/media-watch/page.jsx
import MediaWatchClient from "@/components/MediaWatchClient";
const SITE_URL = "https://journalism-society.vercel.app";

export const metadata = {
  title: "Media Watch — Track All Media Coverage | Journalism Society",
  description:
    "Journalism Society's Media Watch tracks and categorises all media coverage across every active docket. Filter by outlet, stance, type, and docket ID. Submit missing coverage for editorial review.",
  keywords: ["media watch India", "media coverage tracking", "journalism accountability India", "press monitoring India"],
  alternates: { canonical: `${SITE_URL}/media-watch` },
  openGraph: {
    title: "Media Watch — Track All Media Coverage | Journalism Society",
    description: "All media coverage across every active docket — tracked, categorised, and cross-referenced for full accountability.",
    url: `${SITE_URL}/media-watch`,
    type: "website",
    images: [{ url: `${SITE_URL}/og-image.jpg`, width: 1200, height: 630, alt: "Journalism Society Media Watch" }],
  },
  twitter: {
    card: "summary_large_image",
    title: "Media Watch | Journalism Society",
    description: "All media coverage across every active docket — tracked and cross-referenced for accountability.",
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { "@type": "ListItem", position: 2, name: "Media Watch", item: `${SITE_URL}/media-watch` },
  ],
};
